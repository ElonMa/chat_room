package com.may.chatroom.multi.server;
...
public class ExecuteClient implements Runnable {
    ...
    private static String STORE_MESSAGE_PATH_NAME = "D:" + File.separator + "may" + File.separator + "chat_room" + File.separator + "StoreMessageOfRegister.txt";//默认将新用户注册信息保存在此文件中
    ...
    private void register(String userName, String password, Socket client) {
        ...
        if ( nLength <= 4 || pLength <= 4) {
            this.sendMessage(client, "输入的用户名或密码长度不符合要求（用户名长度8到15，密码长度6到10）");
            return;
        }

        File file = new File("D:" + File.separator + "may" + File.separator + "chat_room" + File.separator + "StoreMessageOfRegister.txt");//用户注册信息保存文件
